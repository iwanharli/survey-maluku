        ...
        <div class="card-body px-0 pt-0">
          <div class="table-responsive">
            <table id="user-list-table" class="table table-hover table-bordered mb-0" role="grid" data-toggle="data-table">
              ...
              <tbody>
                <tr v-for="(item, index) in paginatedSurveyData" :key="index">
                  ...
                  <td class="text-center bg-soft-secondary">
                    <button type="button" class="btn btn-md btn-primary" data-bs-toggle="modal" data-bs-target="#modalAnswer" @click="showSurveyDetails(item.responden_answers)">HASIL SURVEY</button>
                  </td>
                </tr>
              </tbody>
            </table>
  ...
  methods: {
    async fetchRespondenSurvey() {
      try {
        ...
        const response = await axios.get('/api/survey/view_respondens_by_survey/all?page=1&limit=2000', config)

        this.respondenData = response.data.data.data
        this.questionAnswerData = response.data.data.data

        this.surveyCount = this.respondenData.length

        console.log('💚 RESPONDEN SURVEY', this.respondenData)
        console.log('💚 QUESTION ANSWER', this.questionAnswerData)
      } catch (error) {
