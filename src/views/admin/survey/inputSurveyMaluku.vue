          ...
          <div class="d-flex align-items-center justify-content-between">
            <b-row>
              ...
              <b-col xl="11" lg="11" md="11" sm="11" class="mt-4">
                <b-button class="btn btn-md mt-3 btnQuestion" v-for="answer in item.answers" :key="answer.id" @click="selectAnswer(index, answer.answer, answer.option)" :variant="answer.selected ? 'primary' : 'outline-primary'">
                  <span v-if="index === 11">
                    <!-- Show photo for index 11 answers -->
                    <img v-if="index === 11" :src="answer.photo" style="width: 150px; height: 100px; margin-left: 10px; border-radius: 10px; border: 2px solid white" />
                    <br />
                    <!-- Line break after the photo -->
...
export default {
  ...
  mounted() {
    this.showSurveyQuestion(), this.fetchProvinces()
  },
  ...
  methods: {
    ...
    async showSurveyQuestion() {
      ...
      try {
        ...
        const response = await axios.get(`/api/survey/get_questions_answers/2`, config)

        this.surveyQuestionData = response.data.data

        // DUMMY PHOTO
        if (this.surveyQuestionData[10]) {
          const dummyPhotos = ['https://img001.prntscr.com/file/img001/XiSIdPcnRs-YK03jVKLZdQ.png', 'https://img001.prntscr.com/file/img001/n0aW1qShS1WKz_3p5_QVqA.png', 'https://img001.prntscr.com/file/img001/-asRdYSvQre2s5syWH09aA.png', 'https://img001.prntscr.com/file/img001/61cDYaxcQJ2Ocd673qYIOg.png', 'https://via.placeholder.com/300?text=TT/TJ']

          this.surveyQuestionData[10].answers.forEach((answer, index) => {
            answer.photo = dummyPhotos[index]
          })
        }
    ...
    async submitSurvey() {
      ...
      await axios
        .post('/api/survey/submit/1', formData, config)
        .then((response) => {
          // console.clear()
          console.log('💚 SURVEY SUBMITTED', response.data.data)
