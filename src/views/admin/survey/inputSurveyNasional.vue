          ...
          <div class="d-flex align-items-center justify-content-between">
            <b-row>
              ...
              <b-col xl="11" lg="11" md="11" sm="11" class="mt-4">
                <b-button class="btn btn-md mt-3 btnQuestion" v-for="answer in item.answers" :key="answer.id" @click="selectAnswer(index, answer.answer, answer.option)" :variant="answer.selected ? 'primary' : 'outline-primary'"> {{ answer.option }}. &nbsp; {{ answer.answer }} </b-button>
              </b-col>
            </b-row>
          </div>
        </b-card>
  ...
  methods: {
    ...
    async showSurveyQuestion() {
      ...
      try {
        ...
        const response = await axios.get(`/api/survey/get_questions_answers/1`, config)
