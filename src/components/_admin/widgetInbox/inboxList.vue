<template>
  <b-col lg="8" xl="9">
    <b-card class="bg-primary" style="border-radius: 20px; border: 5px solid white" v-if="detailInboxPanel === false">
      ...
      <hr />
      <div class="card-body text-dark mt-4" style="border-radius: 20px; background: #f0f0f0" v-for="(inbox, index) in inboxData" :key="index++">
        <a href="#" class="inbox-custom" @click="showDetail(inbox.from, inbox.from_user.id)" style="text-decoration: none">
          <b-row>
            ...
            <b-col xl="7" class="mt-1">
              <h6 style="font-weight: bolder; text-transform: capitalize">{{ inbox.messages.title }}</h6>
              <span>{{ inbox.messages.message_html }} </span>
            </b-col>
